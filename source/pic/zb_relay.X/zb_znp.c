...
#define ZNP_RST_TRIS	TRISBbits.TRISB3
#define ZNP_RST_PORT	PORTBbits.RB3
...
void znp_frame_parser(uint8_t ch) {
    ...
    switch (znp_state) {
        ...
        case FCS_STATE:
            if (znp_fcs == ch) {
                last_cmd_rcv
                        = BUILD_UINT16(znp_recv_msg.cmd1, znp_recv_msg.cmd0);
                _zb_znp_cb(
                           last_cmd_rcv,
                           znp_recv_msg.data,
                           znp_recv_msg.len
...
void znp_af_data_request(uint16_t dst_addr,
    ...
    fcs_calc ^= 10 + len;
    fcs_calc ^= len;
    fcs_calc ^= LSB(dst_addr);
    fcs_calc ^= MSB(dst_addr);
    ...
    usart_send(LSB(dst_addr));
    usart_send(MSB(dst_addr));
    usart_send(endpoint);
    usart_send(endpoint);
